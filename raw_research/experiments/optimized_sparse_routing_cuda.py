...
def run_comparison(model_name="unsloth/Llama-3.2-1B", max_new_tokens=20):
    ...
    # First load the tokenizer which doesn't have the rope_scaling issue
    ...
    try:
        ...
        if 'rope_scaling' in str(e):
            print("Fixing rope_scaling configuration for Llama 3...")
            ...
            if hasattr(config, 'rope_scaling') and isinstance(config.rope_scaling, dict):
                ...
                if 'rope_type' in rope_config and 'llama3' in rope_config.get('rope_type', ''):
                    # Convert to the format expected by transformers
                    config.rope_scaling = {
                        'type': 'linear',  # Use linear scaling type
                        'factor': float(rope_config.get('factor', 2.0))
                    }
            
            # Now load with fixed config
            model = MatrixFreeLlamaForCausalLM.from_pretrained(
                model_name,
                config=config,
                torch_dtype=precision,
            ...
            # Re-raise if it's not a rope_scaling issue
            raise
